...
var _articlePostPC = {
  ...
  _post: function(id, isprev) {
    ...
    if (_articlePostPC._isprev == 1) {
      ...
      //_$(_postApi._article, "update_id=" + (id || "") + "&title=" + _._encode(_title) + "&content=" + _._encode(_content) + "&private=" + _private + "&scene_id=" + _post._sceneId + "&verify_code=" + _yzm, _articlePostPC._ok);
    	var posturl = "/hdbweb/index.php/article/add_post";
    	$.post(posturl,{"post_content":_._encode(_content),"post_title":_._encode(_title),"term":"test"},function(data){
				if(data.status==1){
					window.location = "/hdbweb/index.php/article/index?id=" + data.id;
				}
				if(data.status==0){
					window.location = "/hdbweb/index.php/list/index"; 
				}
  ...
  _ok: function(json, code) {
    ...
    if ((navigator.userAgent.indexOf('MSIE') >= 0) && (navigator.userAgent.indexOf('Opera') < 0)) {
      ...
      _g("{:U('article/index', array('id'=>json.article_id)}"); //跳转到文章详情页面
    }
    if (_articlePostPC._winOpen != null) {
      _articlePostPC._winOpen.close();
    }
